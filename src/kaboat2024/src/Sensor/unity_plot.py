...
# 전역 변수로 temp 리스트 선언
temp = [1000] * num
angles2 = np.radians(np.arange(num//2))//6
angles1= -np.radians(np.arange(num//2))//6
...
def imu_callback(data):
    ...
    heading_angle = data.data+180 # 방위각 업데이트
    desired_angle = 400
...
def destination_callback(data):
    global destination
    destination[0]=data.point.x
    destination[1]=data.point.y
    print(destination)
def update_plot(frame):
    ...
    ax.set_xlim(-40, 80)  # x축 범위 설정
    ax.set_ylim(-15, 60)  # y축 범위 설정
    ...
    line_x_end = gps_position[0] + line_length * np.sin(np.radians(heading_angle+180))
    line_y_end = gps_position[1] + line_length * np.cos(np.radians(heading_angle+180))
    ...
    if desired_angle != 400:
        ...
        print(np.radians(desired_angle))
    ...
    # # IMU 방향 표시 polar_ax = fig.add_subplot(122, projection='polar')  # 1행 2열 중 두 번째, 극좌표계
